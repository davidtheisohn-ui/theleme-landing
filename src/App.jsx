...
{/* VALEURS */}
            ...
            <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="rounded-2xl border border-black/10 bg-white p-5 lg:col-span-2">
                ...
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="rounded-xl border border-black/10 bg-white p-3"><p className="text-neutral-600">Pièces</p><div className="mt-2">
      <ul className="divide-y divide-black/5">
        {Object.entries(rooms).filter(([k,q])=>q>0).map(([k,q])=>{
          ...
          return (
            <li key={k} className="flex items-center gap-3 py-1.5 text-sm">
              <span className="inline-block w-2 h-2 rounded-full bg-black"></span>
              <span>{labels[k]||k}</span>
              <span className="ml-auto font-semibold">×{q}</span>
            </li>
          )
        })}
        {Object.entries(rooms).filter(([k,q])=>q>0).length===0 && (
